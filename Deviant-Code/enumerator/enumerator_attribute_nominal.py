...
def compute_full_support_nominal_with_bitset(set_indices_prec,datasetIndices_bitset,attr,closed=True):
    ...
    attr_pattern=attr['pattern']
    index_attr_bitset=attr['index_attr_bitset']
    return set_indices_prec&set.union(*(attr['index_attr'][x] for x in filter(partial(is_not, None), attr_pattern)))  , datasetIndices_bitset&reduce(or_,[index_attr_bitset[x] for x in filter(partial(is_not, None), attr_pattern)])
